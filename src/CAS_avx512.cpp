...
void CAS::filter_avx512(PVideoFrame& src, PVideoFrame& dst, const CAS* const __restrict, IScriptEnvironment* env) noexcept
{
    ...
    auto filtering = [&](const vec_t a, const vec_t b, const vec_t c, const vec_t d, const vec_t e, const vec_t f, const vec_t g, const vec_t h, const vec_t i,
        const Vec16f chromaOffset) noexcept {
            ...
            //  w 1 w
            //  0 w 0
            const Vec16f weight = amp * sharpness_;
            if constexpr (std::is_integral_v<pixel_t>)
    ...
    for (int i = 0; i < planecount; i++)
    {
        ...
        if (process[i] == 3)
        {
            ...
            const Vec16f chromaOffset = i ? 1.0f : 0.0f;
...
template void CAS::filter_avx512<uint8_t>(PVideoFrame& src, PVideoFrame& dst, const CAS* const __restrict, IScriptEnvironment* env) noexcept;
template void CAS::filter_avx512<uint16_t>(PVideoFrame& src, PVideoFrame& dst, const CAS* const __restrict, IScriptEnvironment* env) noexcept;
template void CAS::filter_avx512<float>(PVideoFrame& src, PVideoFrame& dst, const CAS* const __restrict, IScriptEnvironment* env) noexcept;
